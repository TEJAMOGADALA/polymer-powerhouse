...
export default defineTool({
  ...
  description:
    "List delivery challans, optionally filtered by company slug or status. Reads the same 'documents' table the dashboard uses so counts always match.",
  inputSchema: {
  ...
  handler: async ({ company_slug, status, limit }, ctx) => {
    ...
    return {
      ...
      structuredContent: { challans: data ?? [], count: data?.length ?? 0 },
    };
  },
});
